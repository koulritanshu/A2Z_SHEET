...
{
public:
    // Segment Tree Template copied from GeeksForGeeks
    ...
    vector<int> countOfPeaks(vector<int> &nums, vector<vector<int>> &queries)
    {
        ...
        int *st = constructST(arr, nums.size());
        vector<int> ans;
        int n = nums.size();
        for (int i = 0; i < queries.size(); i++)
        {
            ...
            if (queries[i][0] == 1)
            {
                if ((queries[i][2] - queries[i][1]) <= 1)
                {
                    ...
                    ans.push_back(getSum(st, nums.size(), queries[i][1] + 1, queries[i][2] - 1));
            }
            else
            {
                int ind = queries[i][1];
